...
const SwipeButton: React.FC<SwipeButtonProps> = ({
  ...
  const handleUnlock = () => {
    ...
    setIsDragging(false);
    setDragWidth(
      containerRef.current ? containerRef.current.clientWidth : 1000
    );
  ...
  const FILL_STYLE: React.CSSProperties = {
    ...
    transition: isDragging
      ? "none"
      : "width 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275)", // Snap back animation
  ...
  return (
    <div ref={containerRef} style={CONTAINER_STYLE}>
      {/* Background Text */}
      <div style={TEXT_STYLE}>{isUnlocked ? "ACCESS GRANTED" : text}</div>
